...
@section('footer_js')
    ...
    <script type="text/javascript">
        var table = $(".data-table").DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('OnHoldSupportTickets') }}",
            columns: [{
                    data: 'DT_RowIndex',
                    name: 'DT_RowIndex'
                },
    ...
    <script>
        ...
        $('body').on('click', '.inProgressBtn', function() {
            var slug = $(this).data("id");
            if (confirm("Are You sure want to Change the Status to In Progress !")) {
                $.ajax({
                    type: "GET",
                    url: "{{ route('ChangeStatusSupportInProgress', '') }}/" + slug,

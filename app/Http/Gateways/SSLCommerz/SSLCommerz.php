...
use App\Http\Gateways\SSLCommerz\SSLCommerzParams;
...
class SSLCommerz extends SSLCommerzParams
{
    ...
    {
        $payment_gateway = DB::table('payment_gateways')->where('provider_name', 'ssl_commerz')->where('status', 1)->first();

        $this->store_id =  $payment_gateway->api_key ?? config('sslcommerz.store.id');
        $this->store_passwd =  $payment_gateway->secret_key ?? config('sslcommerz.store.password');
    ...
    public static function validate_payment($request)
    {
        ...
        if (self::SSLC_Hash_Verify($request)) {
            ...
            $storeid = $payment_gateway->api_key ?? config('sslcommerz.store.id');
            $storepass = $payment_gateway->secret_key ?? config('sslcommerz.store.password');
    ...
    private static function SSLC_Hash_Verify($request)
    {
        ...
        if (isset($request) && isset($request->verify_sign) && isset($request->verify_key)) {
            ...
            $new_data['store_passwd'] = md5($payment_gateway->secret_key ?? config('sslcommerz.store.password'));
            ksort($new_data);
            $hash_string = "";

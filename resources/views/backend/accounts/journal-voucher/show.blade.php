                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="card-tools">
                            ...
                            <a href="{{ route('voucher.journal.edit', $journalVoucher->id) }}" class="btn btn-primary">
                                <i class="fas fa-edit"></i> Edit
                            </a>
                            <a href="{{ route('voucher.journal.print', $journalVoucher->id) }}" class="btn btn-info"
                                target="_blank">
                            ...
                            <div class="col-md-6">
                                <table class="table table-bordered">
                                    <tr>
                                        ...
                                        <td>{{ $journalVoucher->voucher_no ?? 'N/A' }}</td>
                                    </tr>
                                    <tr>
                                        <th>Date:</th>
                                        <td>{{ $journalVoucher->trans_date ? (is_string($journalVoucher->trans_date) ? \Carbon\Carbon::parse($journalVoucher->trans_date)->format('d/m/Y') : $journalVoucher->trans_date->format('d/m/Y')) : 'N/A' }}
                                        </td>
                                    </tr>
                                    <tr>
                                        <th>Total Amount:</th>
                                        <td>{{ number_format($journalVoucher->amount ?? 0, 2) }}</td>
                                    </tr>
                                    <tr>
                                        <th>Status:</th>
                                        <td>
                                            <span
                                                class="badge badge-{{ $journalVoucher->status ?? 0 ? 'success' : 'danger' }}">
                                                {{ ($journalVoucher->status ?? 0) == 1 ? 'Active' : 'Inactive' }}
                                            </span>
                                        </td>
                                    </tr>
                                    <tr>
                                        <th>Remarks:</th>
                                        <td>{{ $journalVoucher->comments ?? 'N/A' }}</td>
                                    </tr>
                                </table>
                            </div>
                        </div>

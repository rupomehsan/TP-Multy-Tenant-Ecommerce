...
<head>
    ...
    <title>Receive Voucher - {{ $receiveVoucher->voucher_no ?? 'N/A' }}</title>
    <style>
        body {
...
<body>
    <div class="voucher-container">
        <div class="header">
            ...
            <div class="voucher-title">RECEIVE VOUCHER</div>
        </div>

        <div class="voucher-info">
            <div class="info-item">
                <div class="info-label">Voucher No:</div>
                <div class="info-value">{{ $receiveVoucher->voucher_no ?? 'N/A' }}</div>
            </div>
            <div class="info-item">
                <div class="info-label">Date:</div>
                <div class="info-value">{{ $receiveVoucher->trans_date ? (is_string($receiveVoucher->trans_date) ? \Carbon\Carbon::parse($receiveVoucher->trans_date)->format('d/m/Y') : $receiveVoucher->trans_date->format('d/m/Y')) : 'N/A' }}</div>
            </div>
            <div class="info-item">
                <div class="info-label">Amount:</div>
                <div class="info-value">{{ number_format($receiveVoucher->amount ?? 0, 2) }}</div>
            </div>
        </div>
        ...
        <div class="total-section">
            <div>
                <strong>Total Amount: {{ number_format($receiveVoucher->amount ?? 0, 2) }}</strong>
            </div>
        </div>

        @if($receiveVoucher->comments)
            <div style="margin-top: 20px;">
                <strong>Remarks:</strong> {{ $receiveVoucher->comments }}
            </div>
        @endif

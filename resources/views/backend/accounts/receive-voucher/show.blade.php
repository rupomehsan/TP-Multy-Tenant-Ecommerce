                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="card-tools">
                            ...
                            <a href="{{ route('voucher.receive.edit', $receiveVoucher->id) }}" class="btn btn-primary">
                                <i class="fas fa-edit"></i> Edit
                            </a>
                            <a href="{{ route('voucher.receive.print', $receiveVoucher->id) }}" class="btn btn-info"
                                target="_blank">
                            ...
                            <div class="col-md-6">
                                <table class="table table-bordered">
                                    <tr>
                                        ...
                                        <td>{{ $receiveVoucher->voucher_no ?? 'N/A' }}</td>
                                    </tr>
                                    <tr>
                                        <th>Date:</th>
                                        <td>{{ $receiveVoucher->trans_date ? (is_string($receiveVoucher->trans_date) ? \Carbon\Carbon::parse($receiveVoucher->trans_date)->format('d/m/Y') : $receiveVoucher->trans_date->format('d/m/Y')) : 'N/A' }}
                                        </td>
                                    </tr>
                                    <tr>
                                        <th>Total Amount:</th>
                                        <td>{{ number_format($receiveVoucher->amount ?? 0, 2) }}</td>
                                    </tr>
                                    <tr>
                                        <th>Status:</th>
                                        <td>
                                            <span
                                                class="badge badge-{{ $receiveVoucher->status ?? 0 ? 'success' : 'danger' }}">
                                                {{ ($receiveVoucher->status ?? 0) == 1 ? 'Active' : 'Inactive' }}
                                            </span>
                                        </td>
                                    </tr>
                                    <tr>
                                        <th>Remarks:</th>
                                        <td>{{ $receiveVoucher->comments ?? 'N/A' }}</td>
                                    </tr>
                                </table>
                            </div>
                        </div>
